...
public class MiniTomcat {
    ...
    private final ApplicationContext springContext; //IoC容器

    public MiniTomcat(int port) throws Exception {
        this.port = port;
        // 1. 初始化Spring容器(扫描con.projrct包)
        ...
        // 3. 从Spring容器中获取所有Controller(所有的带@Controller注解的Bean)
        Map<String, Object> controllers=springContext.getBeansWithAnnotation(Controller.class);
        for (Object controller : controllers.values()) {
            // 注册Controller中的方法
            routeScanner.registerController(controller);
        }
        System.out.println("MiniTomcat 初始化完成，已从Spring容器注册所有Controller");
    }
